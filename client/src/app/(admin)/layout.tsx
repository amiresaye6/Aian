...
function Sidebar({ collapsed, setCollapsed }: { collapsed: boolean; setCollapsed: (v: boolean) => void }) {
  ...
  return (
    <aside
      className={cn(
        "relative hidden shrink-0 border-r border-indigo-500/10 bg-[color:var(--surface)]/60 backdrop-blur-xl transition-[width] duration-300 md:flex md:flex-col",
        collapsed ? "w-[76px]" : "w-[260px]",
      )}
    >
      <div className="flex items-center justify-between p-4">
        <Link href="/admin" className={cn("flex items-center gap-2.5", collapsed && "justify-center w-full")}>
          <AianMark className="h-7 w-7 text-indigo-500" />
          {!collapsed && (
            <span className="font-display text-[15px] font-semibold tracking-[0.18em] text-indigo-500 flex items-center gap-2">
      ...
      <nav className="mt-6 flex-1 overflow-y-auto scrollbar-none space-y-0.5 px-3 pb-6">
        {!collapsed && (
          <div className="px-2 pb-2 text-[10px] font-semibold uppercase tracking-[0.22em] text-indigo-500/70">
            Platform Management
          </div>
        )}
        {NAV.map((item) => {
          ...
          return (
            ...
            >
              ...
              <item.icon className={cn("h-[18px] w-[18px] shrink-0", active && "text-indigo-500")} />
    ...
    <header className="sticky top-0 z-30 flex h-16 items-center gap-3 border-b border-white/5 bg-[color:var(--background)]/70 px-4 backdrop-blur-xl md:px-6">
      ...
      <div className="ml-auto flex items-center gap-2">
        ...
        <div className="ml-1 flex items-center gap-2.5 rounded-xl border border-white/10 bg-white/[0.03] py-1 pl-1 pr-3">
          <div className="relative flex h-7 w-7 shrink-0 items-center justify-center overflow-hidden rounded-lg bg-indigo-500 text-[12px] font-bold text-white">
            <img
          ...
          <div className="hidden text-left leading-tight sm:block">
            ...
            <div className="text-[10.5px] text-indigo-500">Super Admin</div>
          </div>
        </div>
        <AlertDialog>
