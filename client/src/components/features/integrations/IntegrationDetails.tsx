...
export function IntegrationDetails({ providerKey }: { providerKey: string }) {
  ...
  useEffect(() => {
    ...
    getSelectedResources(providerKey, cid)
      .then((res) => {
    ...
    getRecentKnowledge(cid)
      .then((res) => {
        setRecentKnowledge(res.data || res);
      })
      .catch(() => {});

    getKnowledgeStats(cid)
      .then((res) => {
    ...
    if (providerKey === "github") {
      ...
      getGithubBreakdown(cid)
        .then((res) => {
          setGithubBreakdown(res);
        })
        .catch(() => {});
    }
    if (providerKey === "zoom") {
      getScheduledMeetings(cid, "zoom")
        .then((res) => {
      ...
      {tab === "overview" && (
        <div className="grid gap-6 lg:grid-cols-3">
              ...
              {providerKey === "zoom" ? (
                <>
                  ...
                  <Metric
                    ...
                    value={
                      scheduledMeetings?.resourcesFound?.toLocaleString() ?? "0"
                    }
                  />
                </>
              ) : (
                <>
                  ...
                  <Metric
                    ...
                    value={
                      providerKey === "jira" ? membersCount.toString() : "0"
                    }
                  />
                  {providerKey === "github" && (
                    <Metric
            ...
            <div className="glass rounded-2xl p-6 bg-white dark:bg-transparent shadow-sm dark:shadow-none border border-black/5 dark:border-white/10">
              ...
              <div className="divide-y divide-black/5 dark:divide-white/5">
                {recentKnowledge.length === 0 ? (
                  <div className="py-4 text-[13px] text-muted-foreground text-center">
                    No recent knowledge ingested.
                  </div>
                ) : (
          ...
          <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="mt-0.5 text-[11.5px] text-muted-foreground">
                    ...
                    {healthData?.lastSyncAt
                      ? formatDistanceToNow(new Date(healthData.lastSyncAt), {
                          addSuffix: true,
                        })
                      : "Never"}
                  </div>
            ...
            <div className="glass rounded-2xl p-6 bg-white dark:bg-transparent shadow-sm dark:shadow-none border border-black/5 dark:border-white/10">
              <h3 className="mb-3 font-display text-[14px] font-semibold tracking-tight text-foreground">
                Danger zone
              </h3>
              <button
              ...
              >
                ...
                {isDisconnecting ? "Disconnecting..." : "Disconnect this Eye"}
              </button>
              <p className="mt-2 text-[11px] text-muted-foreground">
                Ingested data is retained per your retention policy and can be
        ...
        <div className="glass rounded-2xl p-5 bg-white dark:bg-transparent shadow-sm dark:shadow-none border border-black/5 dark:border-white/10">
          ...
          <div className="grid gap-2 sm:grid-cols-2">
            {resources?.length > 0 ? (
              resources.map((r: any) => (
                ...
                >
                  <div className="flex h-9 w-9 items-center justify-center rounded-lg border border-black/10 dark:border-white/10 bg-black/[0.03] dark:bg-white/[0.03] text-[color:var(--gold-soft)]">
                    {r.isPrivate || r.metadata?.private ? (
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="inline-flex items-center gap-1 text-[11px] text-muted-foreground">
                      ...
                      {r.metadata?.memberCount ?? 0} · active
                    </div>
                  </div>
              ...
              <div className="col-span-2 text-center text-[13px] text-muted-foreground py-4">
              ...
              <div className="flex items-center justify-between mb-4">
                <div>
                  ...
                  <p className="text-[12px] text-muted-foreground mt-0.5">
                    {meetingsData?.resourcesFound ?? meetingsList.length}{" "}
                    meeting(s) found
                  </p>
                </div>
              </div>
                ...
                {meetingsList.length > 0 ? (
                  meetingsList.map((m: any) => {
                    const startTime = m.metadata?.start_time
                      ? new Date(m.metadata.start_time)
                      : null;
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          {providerKey === "jira" ? (
            [
              {
                label: "Issues",
                value: stats?.breakdown?.entities?.toLocaleString() ?? "0",
              },
              {
                label: "Comments",
                value: stats?.breakdown?.messages?.toLocaleString() ?? "0",
              },
              {
                label: "Transitions",
                value: stats?.breakdown?.documents?.toLocaleString() ?? "0",
              },
            ].map((c) => (
            ...
            [
              {
                label: "Pull requests",
                value: githubBreakdown?.pullRequests?.toString() ?? "0",
              },
              {
                label: "Issues & comments",
                value: githubBreakdown?.issuesAndComments?.toString() ?? "0",
              },
              {
                label: "Commits",
                value: githubBreakdown?.commits?.toString() ?? "0",
              },
            ].map((c) => (
              <Metric key={c.label} label={c.label} value={c.value} />
            ))
