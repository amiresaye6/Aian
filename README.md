...
to do:
new employee account must be verified, how and wehre?
add new role pag, the dark mode need to be fixed
the eye settigns supmit button is not working.
aian dashboard has no real data in it at all, just one or two parts only.
ai portal needs messages history by user account and messages limit> for example 20 per day
add filter by type to the extracted artifactes table.
meetings page >> we need to make sure it is workgin and functional "not just a temp place holder."
we need to maek the reports page.
link the sync all now button in the integrations page with teh backend, callign all connected providers sync now endpoint.
...
make a profile page, showing user detials, current system usage, accoutn level of access, limits, stuff like that.
redesign the mimbers page to best sute the rest of the system theme. and layout.
